...
class Database
{
	...
	{
		if ($this->_stmtErrno)
			Log::F()->error('Database error! ' . $this->_stmtError . ' (code: ' . $this->_stmtErrno . ')');
	}
	 ...
	 *
	 * @return Database new mysqlidb object
	 */
	 ...
	 */
	private function conditionToSql($operator, mixed $val)
	{
		switch (strtolower($operator)) {
			case 'not in':
			case 'in':
